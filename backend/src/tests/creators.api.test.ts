...
beforeEach(async () => {
  ...
  await prisma.allowedEmail.deleteMany({
    where: { email: { startsWith: "creator-" } },
  });
...
test("creator library requires auth for reads and admin rights for writes", async () => {
  await withServer(async (baseUrl) => {
    ...
    await createUserAccount({
      email: "creator-member@example.com",
      password: "password123",
      name: "Member",
    });
    ...
    const memberCookie = await loginAndGetCookie(baseUrl, {
      email: "creator-member@example.com",
      password: "password123",
    });
    ...
    const clipCreateResponse = await fetch(
      `${baseUrl}/api/v1/creators/${created.creator.id}/clips`,
      {
        method: "POST",
        headers: { cookie: adminCookie },
        body: clipForm,
    ...
    const clipCreated = (await clipCreateResponse.json()) as {
      clip: { id: string; tags: string[]; title: string | null };
    };
    ...
    const listResponse = await fetch(`${baseUrl}/api/v1/creators`, {
      headers: { cookie: memberCookie },
    });
    ...
    const list = (await listResponse.json()) as {
      creators: Array<{
        id: string;
        clipCount: number;
        clips: Array<{ id: string; tags: string[] }>;
      }>;
    };
    const listedCreator = list.creators.find(
      (creator) => creator.id === created.creator.id,
    );
    ...
    const memberUpdateResponse = await fetch(
      `${baseUrl}/api/v1/clips/${clipCreated.clip.id}`,
      {
        method: "PATCH",
        headers: { cookie: memberCookie, "content-type": "application/json" },
        body: JSON.stringify({
          title: "Updated launch clip",
          tags: ["founder", "demo", "founder"],
        }),
      },
    );
    assert.equal(memberUpdateResponse.status, 403);

    const clipUpdateResponse = await fetch(
      `${baseUrl}/api/v1/clips/${clipCreated.clip.id}`,
      {
        method: "PATCH",
    ...
    const updatedClip = (await clipUpdateResponse.json()) as {
      clip: { title: string | null; tags: string[] };
    };
    ...
    const afterDelete = await fetch(`${baseUrl}/api/v1/creators`, {
      headers: { cookie: memberCookie },
    });
    const afterDeleteJson = (await afterDelete.json()) as {
      creators: Array<{ id: string; clipCount: number }>;
    };
    const creatorAfterDelete = afterDeleteJson.creators.find(
      (creator) => creator.id === created.creator.id,
    );
