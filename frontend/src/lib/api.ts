...
export async function api<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${API_BASE}/api/v1${path}`, { ...init, credentials: 'include', headers: { ...(init.body && !isFormData && !isBinaryBody && !isUrlSearchParams ? { 'Content-Type': 'application/json' } : {}), ...init.headers } });
  if (response.status === 204) return undefined as T;
  const data = await response.json().catch(() => ({})) as { error?: { code?: string; message?: string; requestId?: string } };
  if (!response.ok) throw new ApiClientError(data.error?.code ?? 'REQUEST_FAILED', data.error?.message ?? 'Request failed', data.error?.requestId);
  return data as T;
}
