...
import type { BillingStatus, BrandProfile, ConceptCard, CreatorRecord, GenerationJob, HookPreferenceExample, ProjectResponse, ProjectSnapshot } from '../types/domain';
...
export default function CampaignDashboardPage() {
  ...
  const load = async () => {
    const [projectResponse, creatorResponse, billingResponse] = await Promise.all([
  ...
  return <main className="min-h-screen bg-[#f6f6f1] text-[#111]">
    ...
    <section className="mx-auto max-w-[1440px] px-6 py-8 sm:px-10 sm:py-12">
      <div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><div className="rounded-[32px] bg-[#111] p-7 text-white shadow-[0_24px_80px_rgba(0,0,0,.14)] sm:p-10"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#b8f36b]">Your content engine</p><h2 className="mt-5 max-w-2xl text-4xl font-black leading-[.96] tracking-[-.06em] sm:text-6xl">Tune the brand. Keep the wins. Render what you need.</h2><p className="mt-5 max-w-xl text-sm leading-6 text-white/65">Update the instructions behind your content without touching videos you have already rendered.</p><div className="mt-8 flex flex-wrap gap-3"><button className="inline-flex items-center gap-2 rounded-full bg-[#b8f36b] px-5 py-3 text-sm font-black text-[#111]" onClick={() => document.getElementById('hooks')?.scrollIntoView({ behavior: 'smooth' })}><Layers3 size={16} />Manage hook library</button>{latestRender ? <span className="inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-3 text-sm text-white/70"><Video size={16} />Previous renders stay safe</span> : null}</div></div><div className="grid grid-cols-2 gap-4"><div className="rounded-[28px] border border-black/8 bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#888]">Hooks</p><p className="mt-4 text-4xl font-black">{concepts.length}</p><p className="mt-1 text-sm text-[#777]">in your library</p></div><div className="rounded-[28px] border border-black/8 bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#888]">Render capacity</p><p className="mt-4 text-4xl font-black">{billing?.videoUsage.remaining ?? '∞'}</p><p className="mt-1 text-sm text-[#777]">videos remaining</p></div></div></div>
      ...
      <section id="hooks" className="mt-6 rounded-[30px] border border-black/8 bg-white p-6 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#777]">03 · Hook library</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Choose what to render</h2><p className="mt-2 text-sm text-[#666]">Select any number of hooks up to your remaining capacity. Existing videos are never replaced.</p></div><div className="flex flex-wrap gap-2"><button className={softButton} onClick={() => void generateSimilar()} disabled={busy === 'hooks'}><Sparkles size={15} />{busy === 'hooks' ? 'Generating…' : 'Generate similar hooks'}</button><button className={buttonClass} onClick={() => void createBatch()} disabled={selected.length === 0 || selected.length > renderCapacity || busy === 'render'}><Video size={15} />{busy === 'render' ? 'Starting…' : `Render ${selected.length || ''} selected`}</button></div></div>{renderCapacity === 0 ? <p className="mt-5 rounded-2xl bg-[#fff3ef] p-4 text-sm font-bold text-[#9a3e29]">You have used this billing period’s render allowance. Upgrade or wait for the next period.</p> : null}<div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{concepts.map((concept) => <ConceptEditor key={concept.id} concept={concept} selected={selected.includes(concept.id)} disabled={busy === `concept:${concept.id}`} onToggle={() => setSelected((current) => current.includes(concept.id) ? current.filter((itemId) => itemId !== concept.id) : current.length < renderCapacity ? [...current, concept.id] : current)} onSave={(hookText, demoOverlayText) => void saveConcept(concept, hookText, demoOverlayText)} />)}</div></section>
      <section className="mt-6 rounded-[30px] border border-black/8 bg-white p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#777]">04 · Render history</p><div className="mt-2 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-black tracking-[-.04em]">Your rendered videos</h2><p className="mt-2 text-sm text-[#666]">Every completed render stays available for download.</p></div><button className={softButton} onClick={() => navigate(`/projects/${id}/render`)}><Video size={15} />Open latest</button></div><div className="mt-5 space-y-3">{project.jobs.filter((job) => job.type === 'RENDER_REELS').map((job, index) => <div key={job.id} className="flex flex-col gap-3 rounded-2xl border border-black/8 bg-[#fafaf8] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">Render {project.jobs.filter((item) => item.type === 'RENDER_REELS').length - index}</p><p className="mt-1 text-xs uppercase tracking-[.12em] text-[#888]">{job.status.toLowerCase()} · {new Date(job.createdAt).toLocaleString()}</p></div><div className="flex flex-wrap gap-2">{renderOutputs(job).map((output, outputIndex) => <a key={output.conceptId} className={softButton} href={output.url} download={`render-${index + 1}-reel-${outputIndex + 1}.mp4`}>Download {outputIndex + 1}</a>)}</div></div>)}{project.jobs.every((job) => job.type !== 'RENDER_REELS') ? <p className="rounded-2xl bg-[#fafaf8] p-5 text-sm text-[#666]">No rendered videos yet. Select one or more hooks above to begin.</p> : null}</div></section>
    </section>
  </main>;
}
