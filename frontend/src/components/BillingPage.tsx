  ...
  return (
    <main className="min-h-screen bg-[#f4f4f0] px-5 py-10 text-[#111] sm:px-8 sm:py-16">
      <section className="mx-auto max-w-5xl">
        ...
        {billing?.hasAccess ? (
          <section className="mt-9 overflow-hidden rounded-[32px] bg-[#111] p-6 text-white shadow-[0_24px_70px_rgba(0,0,0,.18)] sm:p-9">
            <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
              <div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#b8f36b]">{billing.accessTier === 'complimentary' ? 'Complimentary ' : ''}{billing.planName} plan{billing.isLegacyPlan ? ' · legacy price retained' : ''}</p><p className="mt-3 text-3xl font-black tracking-[-.05em]">{billing.videoUsage.remaining ?? 'Unlimited'} renders remaining</p><p className="mt-2 text-sm text-white/60">Resets {billing.videoUsage.periodEnd ? new Date(billing.videoUsage.periodEnd).toLocaleDateString() : 'with your next billing period'}{billing.complimentaryAccess?.expiresAt ? ` · Access ends ${new Date(billing.complimentaryAccess.expiresAt).toLocaleDateString()}` : billing.accessTier === 'complimentary' ? ' · No access expiration' : ''}.</p></div>
              {billing.accessTier === 'subscriber' ? <button type="button" onClick={() => void openPortal()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-5 py-3 text-sm font-bold hover:bg-white/10 disabled:opacity-50"><CreditCard size={16} />Manage payment</button> : billing.accessTier === 'complimentary' ? <span className="rounded-full border border-[#b8f36b]/40 bg-[#b8f36b]/10 px-4 py-2 text-xs font-black uppercase tracking-[.14em] text-[#b8f36b]">Granted access</span> : null}
            </div>
        ...
        <div className="mt-7 grid gap-5 md:grid-cols-2">
          {plans.map((plan) => {
            ...
            return <article key={plan.id} className={`relative overflow-hidden rounded-[30px] border p-7 transition sm:p-9 ${active || (!billing?.hasAccess && selected) ? 'border-[#111] bg-white shadow-[0_20px_60px_rgba(0,0,0,.1)]' : 'border-black/10 bg-[#fafaf8]'}`}>
              ...
              {!billing?.hasAccess ? <button type="button" aria-pressed={selected} onClick={() => setSelectedPlanId(plan.id)} className={`mt-8 w-full rounded-full px-5 py-3 text-sm font-black ${selected ? 'bg-[#111] text-white' : 'border border-black/15 bg-white'}`}>{selected ? 'Selected' : `Choose ${plan.name}`}</button> : billing.accessTier === 'subscriber' && !active ? <button type="button" disabled={loading || Boolean(billing.scheduledPlanId)} onClick={() => void changePlan(plan.id)} className="mt-8 w-full rounded-full border border-black/15 bg-white px-5 py-3 text-sm font-black disabled:opacity-40">{plan.videoLimit > (billing.videoUsage.limit ?? 0) ? 'Upgrade now' : 'Switch at renewal'}</button> : null}
            </article>;
          })}
        </div>
        ...
        {billing?.accessTier === 'subscriber' && !billing.cancelAtPeriodEnd ? <button type="button" onClick={() => setShowCancel(true)} className="mx-auto mt-7 block text-sm font-medium text-[#777] underline">Cancel subscription</button> : null}
