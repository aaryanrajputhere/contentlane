...
const inputClass =
  "w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-[#111111] outline-none transition placeholder:text-[#9a9a9a] focus:border-black focus:ring-2 focus:ring-black/5";
const buttonClass =
  "inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#111111] px-5 py-3 text-sm font-semibold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-50";
    ...
    <main className="grid min-h-screen place-items-center overflow-hidden bg-[#f7f6f2] px-6 py-12 text-[#111111]">
      ...
      <div className="relative w-full max-w-[28rem] rounded-[32px] border border-black/8 bg-white p-8 shadow-[0_24px_80px_rgba(0,0,0,0.08)]">
        ...
        <h1 className="mt-6 text-[2.3rem] font-extrabold tracking-[-0.06em]">
          {isSignup ? "Create your beta account" : "Sign in to ContentLane"}
        </h1>
        <p className="mt-3 text-sm leading-7 text-[#666666]">
          {isSignup
            ? "Only approved beta emails can create accounts."
            : "Use the email address that has been approved for the beta."}
        </p>

        <div className="mt-8 space-y-4">
          {isSignup ? (
            <label className="block">
              <span className="mb-2 block text-[11px] font-semibold uppercase tracking-[0.18em] text-[#666666]">
                Name
              </span>
              <input
                value={name}
