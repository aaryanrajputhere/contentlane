...
export default function ProjectRenderPage() {
  ...
  const concepts = useMemo(() => {
    ...
    return source.sort((a, b) => a.sortOrder - b.sortOrder).slice(0, 8);
  ...
  return <main className="min-h-screen bg-[#fafaf8] pb-16 text-[#111]">
    ...
    <section className={`${shell} pt-12`}>
      <div className="text-center"><div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-medium"><Server size={15} />Video batch</div><h1 className="mx-auto mt-7 max-w-[15ch] text-[clamp(3rem,6vw,5.2rem)] font-black leading-[.94] tracking-[-.07em]">Your {reels.length} Reels are taking shape.</h1><p className="mx-auto mt-4 max-w-2xl text-[1.05rem] leading-7 text-[#666]">This batch is preserved in your campaign history. Start another one from the dashboard whenever you want.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><button type="button" onClick={() => navigate(`/projects/${id}/dashboard`)} className={white}>Create another batch</button><button type="button" onClick={() => void startRender()} disabled={starting || renderInProgress || isCompleted(job) || reels.length === 0} className={black}>{starting || renderInProgress ? <Loader2 size={16} className="animate-spin" /> : <Server size={15} />}{starting || renderInProgress ? `Rendering ${progress}%` : isCompleted(job) ? 'Render complete' : 'Render on server'}</button><button type="button" onClick={() => void downloadAll()} disabled={outputs.length === 0 || downloading} className={white}><Download size={16} />{downloading ? 'Preparing ZIP…' : 'Download all'}</button></div></div>
