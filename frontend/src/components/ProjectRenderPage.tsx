...
type Reel = { conceptId: string; clipId: string; creatorName: string; clipUrl: string; sortOrder: number };
type Output = { conceptId: string; creatorName: string; url: string; sortOrder: number; format: string };
...
export default function ProjectRenderPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const existing = projectResponse.project.jobs.find((item) => item.type === 'RENDER_REELS' && (item.status === 'QUEUED' || item.status === 'ACTIVE'));
  ...
  const concepts = useMemo(() => (project?.concepts ?? []).filter((concept) => concept.reviewDecision === 'LIKED').sort((a, b) => a.sortOrder - b.sortOrder).slice(0, 8), [project?.concepts]);
  ...
  const reels: Reel[] = assignments.flatMap((item) => item.clip ? [{ conceptId: item.concept.id, clipId: item.clip.id, creatorName: item.creator.name, clipUrl: item.clip.url, sortOrder: item.concept.sortOrder }] : []);
  ...
  async function startRender() {
    ...
    try {
      const response = await api<{ job: GenerationJob }>(`/projects/${id}/render`, {
        method: 'POST',
        body: JSON.stringify({
          conceptIds: reels.map((reel) => reel.conceptId),
          assignments: reels.map((reel) => ({ conceptId: reel.conceptId, clipId: reel.clipId })),
        }),
      });
      setJob(response.job);
  ...
  return <main className="min-h-screen bg-[#fafaf8] pb-16 text-[#111]">
    ...
    <section className={`${shell} pt-12`}>
      <div className="text-center"><div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-medium"><Server size={15} />Queued video pipeline</div><h1 className="mx-auto mt-7 max-w-[15ch] text-[clamp(3rem,6vw,5.2rem)] font-black leading-[.94] tracking-[-.07em]">Your {reels.length} Reels are taking shape.</h1><p className="mx-auto mt-4 max-w-2xl text-[1.05rem] leading-7 text-[#666]">Creator hooks flow into the full product demo and return as downloadable MP4 files.</p><div className="mt-6 flex justify-center gap-3"><button type="button" onClick={() => void startRender()} disabled={starting || (job !== null && !failed) || reels.length === 0} className={black}>{starting || (job && !failed) ? <Loader2 size={16} className="animate-spin" /> : <Server size={16} />}{job && !failed ? `Rendering ${progress}%` : 'Render on server'}</button><button type="button" onClick={() => void downloadAll()} disabled={outputs.length === 0 || downloading} className={white}><Download size={16} />{downloading ? 'Preparing ZIP…' : 'Download all'}</button></div></div>
