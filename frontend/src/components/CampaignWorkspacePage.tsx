...
function RenderedContent({ rendered, conceptById }: { rendered: ReelOutput[]; conceptById: Map<string, ConceptCard> }) {
  ...
  return <section className="rounded-[30px] border border-black/8 bg-white p-6 sm:p-8"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#888]">Rendered videos</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Your finished content</h2></div><div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{rendered.map((output, index) => { const title = conceptById.get(output.conceptId)?.hookText ?? 'Rendered video'; return <article key={`${output.conceptId}-${output.url}`} className="overflow-hidden rounded-[26px] border border-black/10 bg-[#111] text-white"><video src={output.url} className="aspect-[9/16] w-full object-cover" controls muted autoPlay loop playsInline preload="metadata" /><div className="flex min-h-[72px] items-center gap-3 p-4"><p className="line-clamp-2 min-w-0 flex-1 text-sm font-bold">{title}</p><a href={output.url} download={`content-${index + 1}.mp4`} aria-label={`Download ${title}`} title="Download video" className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/15 bg-white/10 text-white transition hover:border-white/30 hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70"><Download size={15} aria-hidden="true" /></a></div></article>; })}</div></section>;
}
